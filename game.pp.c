...
#define HAND_SIZE 8
#define MAX_CARD 52
#define LOG_FILE_NAME "game_log_sort.txt"
...
// 自动理牌：先按点数从大到小，同点数再按花色（黑桃 > 红心 > 梅花 > 方块）
int compareCardForSorting(const void *a, const void *b) {
    const Card *ca = (const Card *)a;
    const Card *cb = (const Card *)b;
...
void sortHand(Player *p) {
    if (p->cardCount <= 1) return;
    qsort(p->hand, p->cardCount, sizeof(Card), compareCardForSorting);
}
...
int main() {
    ...
    logMsg("========================================\n");
    logMsg("欢迎来到纸牌小游戏（自动理牌 + 自动轮流出牌 + 牌型比较 + 详细日志版）！\n");
    logMsg("========================================\n");
    ...
    for (int i = 0; i < game.playerCount; i++) {
        ...
        logMsg("  %s 的手牌已按“点数从大到小、同点数按黑桃>红心>梅花>方块”排序。\n",
               game.players[i].name);
    }
